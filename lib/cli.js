...
const loadPlugins = () => {
    const modulesDir = path.resolve(path.dirname(getCallerFile()), './node_modules/@clitools');
    fs.readdirSync(modulesDir)
        .forEach((dir) => {
            if (dir !== 'base') {
                const { main, keywords } = fs.readJsonSync(path.resolve(modulesDir, './' + dir, './package.json'));
                if (_.includes(keywords, '@clitools/plugin'))
                    require(path.resolve(modulesDir, './' + dir, main));
            }
        });
};
